...
interface ArticleJOINTagDAO {
    ...
    @Query("SELECT * FROM Article INNER JOIN article_x_tag ON Article.articleID = article_x_tag.articleID WHERE article_x_tag.tagID=:articleID")
    fun getArticleOfTags(articleID:Int): LiveData<List<Article>>

    @Query("SELECT * FROM Tag INNER JOIN article_x_tag ON Tag.tagID = article_x_tag.tagID WHERE article_x_tag.tagID=:tagID")
